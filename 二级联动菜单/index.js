var selectObj = {
    ...
    pro: document.getElementsByClassName('pro')[0],
    citys: document.getElementsByClassName('city')[0],
    selPro: document.getElementsByClassName('selPro')[0],
    dargDown: document.getElementsByClassName('dargDown')[0],
    ...
    showDargDown: function () {
        var self = this;
        this.select.addEventListener('click', function () {
            if (self.show) {
                console.log(1)
                self.show = false;
                self.dargDown.style.display = 'none';
                self.icon.style.backgroundPosition = '0 ' + '-2067px';
            } else {
                self.show = true;
                self.dargDown.style.display = 'block';
                self.icon.style.backgroundPosition = '0 ' + '-2044px';
            }
        })
    },
    renderList: function () {
        this.proUl = this.pro.getElementsByTagName('ul')[0];
        var len = this.province.length;
        var str = '';
        for (var i = 0; i < len; i++) {
            str += '<li>' + this.province[i] + '</li>';
        }
        this.proUl.innerHTML = str;
    },
    hoverCity: function () {
        this.proLi = this.proUl.getElementsByTagName('li');
        var len = this.proLi.length;
        for(var i = 0; i < len; i++){
            this.proLi[i].index = i;
            this.proLi[i].addEventListener('mouseenter',this.rendercity.bind(this))
        }
    },
    rendercity: function(e){
        var index = e.target.index;
        var cityList = this.city[index];
        var len = cityList.length;
        this.cityUl = this.citys.getElementsByTagName('ul')[0];
        var str = '';
        for(var i = 0; i < len; i++){
            str += '<li>' + cityList[i][1] + '</li>'; 
        }
        this.cityUl.innerHTML = str;
        this.cityLi = this.cityUl.getElementsByTagName('li');
        for(var i = 0; i < this.cityLi.length ; i++){
            this.cityLi[i].addEventListener('click',this.choice.bind(this));
        }
    },
    choice:function(e){
        var str = e.target.innerHTML;
        this.selPro.innerHTML = str;
        this.dargDown.style.display = 'none';
        this.icon.style.backgroundPosition = '0 -2067px';
        this.show = false;
    },
    init: function () {
        this.bindEvent();
